...
namespace mirror {
    ...
    void Logger::configure(uint16_t port, const std::string &componentName) {
        ...
        s_SocketContext = zmq::context_t(1, 1);
        m_LogServerSocket = zmq::socket_t(s_SocketContext, zmq::socket_type::stream);
    ...
    void Logger::initializeKeepAliveThread() {
        ...
        // Spawns and detaches thread that sends keep alive message every 15 minutes
        std::thread(
                []() {
                    while (true) {
